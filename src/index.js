...
import BpmnlintModule, { linting } from 'bpmn-js-bpmnlint';
import StaticResolver from 'bpmnlint/lib/resolver/static-resolver';

import PluginManagerModule from './features/plugin-manager';
...
const diagram = `
<?xml version="1.0" encoding="UTF-8"?><bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:zeebe="http://camunda.org/schema/zeebe/1.0" xmlns:bioc="http://bpmn.io/schema/bpmn/biocolor/1.0" xmlns:color="http://www.omg.org/spec/BPMN/non-normative/color/1.0" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" xmlns:modeler="http://camunda.org/schema/modeler/1.0" id="Definitions_01p3jf9" targetNamespace="http://bpmn.io/schema/bpmn" exporter="Camunda Web Modeler" exporterVersion="ed2fcc0" modeler:executionPlatform="Camunda Cloud" modeler:executionPlatformVersion="8.0.0" camunda:diagramRelationId="a0d0615c-3547-410d-a6d0-e0f5f7c190e8" xmlns:camunda="http://camunda.org/schema/1.0/bpmn">
  <bpmn:process id="template-error-handling" name="Error Handling Example" isExecutable="true">
    ...
    <bpmn:serviceTask id="Activity_1kpg3ok" name="Verify Data is Correct">
      <bpmn:extensionElements>
        <zeebe:taskDefinition type="VerityData"/>
      </bpmn:extensionElements>
    ...
    <bpmn:subProcess id="Activity_17oxyev" name="Process Data">
      ...
      <bpmn:serviceTask id="Activity_084s4zz" name="Create account in Support System">
        <bpmn:extensionElements>
          <zeebe:taskDefinition type="CreateSupportAccount"/>
        </bpmn:extensionElements>
        <bpmn:incoming>Flow_0oytcxl</bpmn:incoming>
        <bpmn:outgoing>Flow_07bctic</bpmn:outgoing>
      </bpmn:serviceTask>
      <bpmn:serviceTask id="Activity_13eovqu" name="Grant access to platform.">
        <bpmn:extensionElements>
          <zeebe:taskDefinition type="GrantPlaformAccess"/>
        </bpmn:extensionElements>
    ...
    <bpmn:serviceTask id="Activity_0h20swu" name="Remove Any Records Created">
      <bpmn:extensionElements>
        <zeebe:taskDefinition type="RemoveCreatedRecords"/>
      </bpmn:extensionElements>
  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="template-error-handling">
      ...
      <bpmndi:BPMNShape id="Activity_0ozn9pl_di" bpmnElement="Activity_13eovqu" bioc:stroke="#000000" bioc:fill="#ffffff" color:background-color="#ffffff" color:border-color="#000000">
...
const modeler = new Modeler({
  ...
  linting: {
    bpmnlint: {
      config: {
        rules: {
          'bpmnlint-plugin-foo/bar': 'error'
        }
      },
      resolver: new StaticResolver({
        'rule:bpmnlint-plugin-foo/bar': () => {
          return {
            check: (node, reporter) => reporter.report(node.get('id'), 'Foo, indeed!')
          }
        }
      })
    }
...
const pluginManager = modeler.get('pluginManager');

const toggleTokenSimulation = document.getElementById('toggle-token-simulation');
const toggleLinting = document.getElementById('toggle-linting');

const updateButtons = () => {
  const activePlugin = pluginManager.getActivePlugin();

  toggleTokenSimulation.classList.remove('active');
  toggleLinting.classList.remove('active');
  
  if (activePlugin === 'tokenSimulation') {
    toggleTokenSimulation.classList.add('active');
  }

  if (activePlugin === 'linting') {
    toggleLinting.classList.add('active');
  }
}

document.getElementById('toggle-token-simulation').addEventListener('click', () => {
  if (pluginManager.getActivePlugin() === 'tokenSimulation') {
    pluginManager.deactivatePlugin('tokenSimulation');
  } else {
    pluginManager.activatePlugin('tokenSimulation');
  }

  updateButtons();
});

document.getElementById('toggle-linting').addEventListener('click', () => {
  if (pluginManager.getActivePlugin() === 'linting') {
    pluginManager.deactivatePlugin('linting');
  } else {
    pluginManager.activatePlugin('linting');
  }
